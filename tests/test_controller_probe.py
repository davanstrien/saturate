...
def test_cut_decay_is_proportional_near_floor():
    """Codex r5 blocker #3: a cut from 3 to the clamped minimum 2 is a 1/3
    reduction — halving the baseline would let flat throughput read as growth
    (their repro: 3 -> 2 -> 2 -> 2 -> 10). Decay scales with the reduction."""
    ctrl = Auto(target_waiting=8, initial=8, step=8, min_limit=2)
    ctrl._best_tok = 9000.0
    new = ctrl.decide(dict(waiting=0, running=3, inflight=3, backpressure=5,
                           successes=3, input_bound=False, tok_s=1000.0), 3)
    assert new == 2 and ctrl._best_tok == 9000.0 * (2 / 3)


def test_queue_pressure_voids_active_probe():
    """Codex r5 medium: an independent queue-driven reduction while a probe is
    settling must void the probe — otherwise the pending revert climbs BACK
    over the reduction (their repro: 40 -> 32 -> 24 -> 32)."""
    ctrl = Auto(target_waiting=8, initial=8, step=8)
    ctrl._best_tok = 5000.0  # plateau-blocked
    limit = 32
    flat = dict(waiting=0, running=16, inflight=32, backpressure=0,
    ...
    for _ in range(ctrl._probe_cooldown):  # drive until a probe fires
        limit = ctrl.decide(dict(flat, inflight=limit), limit)
    assert ctrl._probe_from is not None
    limit = ctrl.decide(dict(flat, waiting=100, inflight=limit), limit)  # queue spike: step down
    assert ctrl._probe_from is None  # probe voided: no revert above the reduction
